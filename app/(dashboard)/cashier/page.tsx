    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="card-floating p-5 group">
          <div className="flex items-center justify-between mb-4">
            <span className="text-xs font-bold text-[#435663]">Menunggu Kasir</span>
          ...
          <div className="flex items-baseline justify-between">
            <span className="text-3xl font-black font-mono text-[#313647]">{pendingOrders.length}</span>
            <span className="badge-custom badge-cream">Siap Ditagih</span>
          </div>
          <p className="text-[11px] text-[#435663] mt-2">Unit selesai servis menunggu pembayaran</p>
        </div>

        <div className="card-floating p-5 group">
          <div className="flex items-center justify-between mb-4">
            <span className="text-xs font-bold text-[#435663]">Faktur Lunas</span>
          ...
          <div className="flex items-baseline justify-between">
            <span className="text-3xl font-black font-mono text-[#313647]">{paidOrders.length}</span>
            <span className="badge-custom badge-steel">Terbayar</span>
          </div>
          <p className="text-[11px] text-[#435663] mt-2">Total transaksi faktur berhasil dituntaskan</p>
        </div>

        <div className="card-floating p-5 group">
          <div className="flex items-center justify-between mb-4">
            <span className="text-xs font-bold text-[#435663]">Total Kas Terkumpul</span>
          ...
          <div className="flex items-baseline justify-between">
            <span className="text-xl font-black font-mono text-[#313647]">{formatRupiah(todayRevenue)}</span>
            <span className="badge-custom badge-sage">Omzet</span>
          </div>
          <p className="text-[11px] text-[#435663] mt-2">Pendapatan bersih kasir yang terealisasi</p>
        </div>
      </div>

      {/* Filter Tabs & Search */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="bg-white border border-[#435663]/20 p-1 rounded-xl flex items-center gap-1 shadow-xs">
          <button
            type="button"
            onClick={() => setActiveTab("PENDING")}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-bold transition-all cursor-pointer ${
              activeTab === "PENDING"
                ? "bg-[#313647] text-[#FFF8D4] shadow-xs"
                : "text-[#435663] hover:text-[#313647]"
            }`}
          >
            <span>Siap Ditagih</span>
            <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-black ${activeTab === "PENDING" ? "bg-[#A3B087] text-[#313647]" : "bg-[#FFF8D4] text-[#313647]"}`}>
          ...
          >
            ...
            <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-black ${activeTab === "PAID" ? "bg-[#A3B087] text-[#313647]" : "bg-[#FFF8D4] text-[#313647]"}`}>
              {paidOrders.length}
            </span>
          </button>
        </div>
        ...
        <div className="w-full sm:w-80 relative">
          ...
          />
          <svg className="w-4 h-4 text-[#435663] absolute left-3.5 top-3.5 pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {isLoading ? (
        ...
        <div className="p-16 text-center card-floating">
          <div className="w-14 h-14 rounded-2xl bg-[#FFF8D4] border border-[#435663]/20 flex items-center justify-center text-2xl mx-auto mb-4">💳</div>
          <p className="text-sm font-bold text-[#313647] mb-1">
            {activeTab === "PENDING" ? "Tidak ada antrean pembayaran" : "Belum ada riwayat faktur lunas"}
          </p>
          <p className="text-xs text-[#435663] max-w-md mx-auto">
            {activeTab === "PENDING"
            ...
            <div key={order.id} className="card-floating p-5 sm:p-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-[#435663]/15">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="font-mono text-xs font-bold px-2.5 py-1 rounded-lg bg-[#313647] text-[#FFF8D4]">{order.orderNumber}</span>
                  <span className="font-mono text-xs font-black px-2.5 py-1 rounded-lg bg-[#A3B087]/25 text-[#313647] border border-[#A3B087]/50">{order.vehicle?.plateNumber}</span>
                  <span className="text-sm font-bold text-[#313647]">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5 py-4 text-xs">
                <div>
                  <p className="text-[10px] font-bold text-[#435663] uppercase tracking-wider mb-1.5">Pelanggan & Kontak</p>
                  <p className="font-bold text-[#313647] text-sm">{order.customer?.name}</p>
                  <a
                    href={`https://wa.me/${order.customer?.phone.replace(/^0/, "62")}`}
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center gap-1.5 text-[#313647] font-semibold hover:underline mt-1 transition-colors"
                  >
                ...
                <div>
                  <p className="text-[10px] font-bold text-[#435663] uppercase tracking-wider mb-1.5">Rincian Pengerjaan</p>
                  <p className="text-[#435663] font-medium">{order.items.length} Tindakan Jasa Servis</p>
                  <p className="text-[#435663] font-medium">{order.parts.length} Item Suku Cadang</p>
                  <p className="text-[11px] text-[#435663] mt-1">
                    Teknisi: <span className="text-[#313647] font-bold">{order.mechanicName || "Mekanik Ahli"}</span>
                  </p>
                </div>

                <div className="md:text-right">
                  <p className="text-[10px] font-bold text-[#435663] uppercase tracking-wider mb-1.5">Total Tagihan Faktur</p>
                  <p className="text-2xl font-black font-mono text-[#313647]">{formatRupiah(order.grandTotal)}</p>
                  <p className="text-[11px] font-mono text-[#435663] mt-1">
