  ...
  return (
    <div className="space-y-8">
      ...
      {stats.lowStockCount > 0 && (
        <div className="p-4 sm:p-5 rounded-2xl bg-white border border-rose-200 shadow-sm flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="text-sm font-bold text-rose-800">
                Peringatan Gudang: {stats.lowStockCount} suku cadang di bawah batas minimum
              </h3>
              <p className="text-xs text-slate-500 mt-0.5">
                Segera lakukan restock agar tidak menghambat pengerjaan servis di pit.
              </p>
              <div className="flex flex-wrap gap-1.5 mt-2.5">
                {stats.lowStockItems.map((item) => (
                  <span
                    key={item.id}
                    className="text-[11px] px-2.5 py-0.5 rounded-full bg-rose-50 text-rose-700 border border-rose-200 font-medium"
                  >
                    {item.name}: sisa <strong className="font-mono font-bold">{item.stock}</strong> {item.unit}
                  </span>
                ))}
              </div>
            </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Link
            href="/inventory/parts"
            className={`card-floating p-5 block cursor-pointer ${
              stats.lowStockCount > 0 ? "!border-rose-300 !bg-rose-50/40" : ""
            }`}
          >
            <div className="flex items-center justify-between mb-2">
